...
const EditPostForm = ({ post }: postProps) => {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const result = await api.patch(`/post/${post._id}`, payload);
      if (result.ok) {
        router.push('/post/');
      } else {
        setErrorsApi(result.error || 'Failed to update post');
      }
    } catch (error) {
      console.error('Update post error', error);
      setErrorsApi('Failed to update post');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full border px-3 py-2 rounded bg-gray-400"
              />
              {errors.author && (
